...
public:
	...
	constexpr float axis_pression(unsigned short pad, Xbox::Axis axis) const {
		...
		if (axis == Xbox::Axis::LeftJoystick_Left || axis == Xbox::Axis::LeftJoystick_Right) {
			return (abs(m_gamepad.at(pad).actual.Gamepad.sThumbLX / 327.67f) < 10) ? 0 : m_gamepad.at(pad).actual.Gamepad.sThumbLX / 327.67f;
		}

		if (axis == Xbox::Axis::LeftJoystick_Up || axis == Xbox::Axis::LeftJoystick_Down) {
			return (abs(m_gamepad.at(pad).actual.Gamepad.sThumbLY / 327.67f) < 10) ? 0 : m_gamepad.at(pad).actual.Gamepad.sThumbLY / 327.67f;
		}

		if (axis == Xbox::Axis::RightJoystick_Left || axis == Xbox::Axis::RightJoystick_Right) {
			return (abs(m_gamepad.at(pad).actual.Gamepad.sThumbRX / 327.67f) < 10) ? 0 : m_gamepad.at(pad).actual.Gamepad.sThumbRX / 327.67f;
		}

		if (axis == Xbox::Axis::RightJoystick_Up || axis == Xbox::Axis::RightJoystick_Down) {
			return (abs(m_gamepad.at(pad).actual.Gamepad.sThumbRY / 327.67f) < 10) ? 0 : m_gamepad.at(pad).actual.Gamepad.sThumbRY / 327.67f;
		}
