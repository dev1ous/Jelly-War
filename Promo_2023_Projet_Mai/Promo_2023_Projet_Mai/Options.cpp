...
Options::Options(FSM_Manager& mgr, WindowManager& window, sf::Font& font, const bool change)
	: FSM{ mgr,window,font,change }, m_optionsButton{ nullptr, sf::RectangleShape() },
	...
	m_volume{ forms::CreateSprite({ (m_sprite.getPosition().x + 5) + (m_sprite.getGlobalBounds().width - 15) * (volumeSound / MAX_VOLUME) - m_volume.getGlobalBounds().width / 2.f,m_sprite.getPosition().y - 60 }, Cache::GetRessources<sf::Texture>("curseur_musique")) },
	m_sprite2{ forms::CreateSprite({800,500}, Cache::GetRessources<sf::Texture>("barre_pleine2")) },
	m_volume2{ forms::CreateSprite({ (m_sprite2.getPosition().x + 5) + (m_sprite2.getGlobalBounds().width - 15) * (volumeBruitages / Max_volume) - m_volume2.getGlobalBounds().width / 2.f,m_sprite2.getPosition().y - 60 }, Cache::GetRessources<sf::Texture>("curseur_musique")) },
	...
	m_indicateur2{ forms::CreateSprite({1150,610},Cache::GetRessources<sf::Texture>("panneau_fond_selection_slimes(259X99)")) },
	current_bar{ forms::CreateSprite({0,0},Cache::GetRessources<sf::Texture>("selection_panneau(1105X100)")) },
	retour_menu{ forms::CreateSprite({1758,0},Cache::GetRessources<sf::Texture>("retour_au_menu_B")) }
{
	...
	FullScreenOff = forms::CreateSprite({ 1150,600 }, Cache::GetRessources<sf::Texture>("selection_non_off(134X134)"));
	FullScreenOn = forms::CreateSprite({ 1275,600 }, Cache::GetRessources<sf::Texture>("selection_oui_off(134X134)"));
	FullScreenOffLu = forms::CreateSprite({ 1150,600 }, Cache::GetRessources<sf::Texture>("selection_non_on(134X134)"));
	FullScreenOnLu = forms::CreateSprite({ 1275,600 }, Cache::GetRessources<sf::Texture>("selection_oui_on(134X134)"));

	m_background.openFromFile("../Ressources/videos/Video_menu.mp4");
	m_background.play();
}

void Options::processEvents(const sf::Event& e)
{
...
void Options::update(const float& dt)
{
	...
	m_volume.setPosition({ (m_sprite.getPosition().x + 5) + (m_sprite.getGlobalBounds().width - 15) * (volumeSound / MAX_VOLUME) - m_volume.getGlobalBounds().width / 2.f, 360 - 60 });
	m_volume2.setPosition({ (m_sprite.getPosition().x + 5) + (m_sprite.getGlobalBounds().width - 15) * (volumeBruitages / Max_volume) - m_volume2.getGlobalBounds().width / 2.f, 500 - 60 });
	...
	case Status::Bruitages:
		...
		if (m_window.hasFocus())
		{
			if (g.is_pressed(0, Xbox::Axis::LeftJoystick_Left))
				volumeBruitages -= 15 * dt;
			else if (g.is_pressed(0, Xbox::Axis::LeftJoystick_Right))
				volumeBruitages += 15 * dt;
		}

		volumeBruitages = std::clamp(volumeBruitages, 0.f, 100.f);

		m_volumeText2 = func::CreateText(std::to_string(static_cast<int>(volumeBruitages)), Cache::GetRessources<sf::Font>("Linebeam"), 30, { m_volume2.getPosition().x + 27,m_volume2.getPosition().y + 30 }, sf::Color::Black);
		break;
	case Status::Sound:
		m_VSyncText.setTexture(Cache::GetRessources<sf::Texture>("VSyncCurrent"));
		m_windowedText.setTexture(Cache::GetRessources<sf::Texture>("pleinEcranCurrent"));
		...
		volumeSound = std::clamp(volumeSound, 0.f, 100.f);

		m_volumeText = func::CreateText(std::to_string(static_cast<int>(volumeSound)), Cache::GetRessources<sf::Font>("Linebeam"), 30, { m_volume.getPosition().x + 27, m_volume.getPosition().y + 30 }, sf::Color::Black);
		break;
